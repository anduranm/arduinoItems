 ...
 * 
 * Code revised and modified by Max Imagination
 * Socials:
 * YouTube - @MaxImagination
 * Instagram - @max.imagination
 * Instructables - Max Imagination
 */
...
int angle = 0;
int radians;
byte rows[8] = {9, 14, 8, 12, 1, 7, 2, 5}; // schematical row pins on LED matrix (do not change)
byte cols[8] = {13, 3, 4, 10, 6, 11, 15, 16}; // schematical column pins on LED matrix (do not change)
byte pins[16] = {5, 4, 3, 2, 14, 15, 16, 17, 13, 12, 11, 10, 9, 8, 7, 6}; // Arduino pins - may need changing according to your wiring
...
void clearWall() { // on-screen entities refresh
  for (int i = 0; i < 8; i++)  screen[i] &= B01111110;
}

void clearGame() { // screen clears
...
void loop() {
  calcWall();
  enterFrameHandler();
  delay(40); // this delay sets the game speed (speed at which the ball moves)
}
...
void face() { // lights specified LEDs on the 8x8 Matrix display to display face at start
  on(1, 1);
  on(1, 2);
  on(2, 1);
  on(2, 2);
...
void reset() { // resets animation after either player fails to deflect the ball and determines angle of ball bouncing
  resetAnim();
  _px = random(3, 5);
  _py = random(3, 5);
